...
// dump($table -> getLastSql());
		 ...
		 *   结果集
		 */
		public function group_select(array $parameter)
		{
			//页码
			$p = intval($_GET['p']) ? intval($_GET['p']) : 1;
			...
			$data['result'] = $table -> table($parameter['tables']) -> where($parameter['where']) -> group($parameter['group'])-> order($parameter['order']) -> field($parameter['field']) -> page($p.',50') -> select();
